...
function upsertReto($pdo, $usuario_id, $reto_id, $progreso, $objetivo) {
    ...
    if ($completado) {
        ...
        if ($nivel_subido) $_SESSION['notificacion_nivel'] = $nivel_subido;
...
$stmt = $pdo->prepare('SELECT id FROM lista_pendientes WHERE usuario_id = ? AND pelicula_id = ?');
$stmt->execute([$usuario_id, $tmdb_id]);
$en_pendientes = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['marcar_vista'])) {
    if (!$ya_vista) {
        ...
        $stmt->execute([$usuario_id, $tmdb_id, $contexto['animo'] ?? null, $contexto['compania'] ?? null, $pelicula['genres'][0]['id'] ?? null, $pelicula['runtime'] ?? 0]);
        // Quitar de pendientes si estaba
        $pdo->prepare('DELETE FROM lista_pendientes WHERE usuario_id = ? AND pelicula_id = ?')->execute([$usuario_id, $tmdb_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['quitar_vista']) && $_POST['quitar_vista'] == '1') {
    ...
    $stmt = $pdo->prepare('SELECT ur.reto_id, ur.completado, r.tipo, r.objetivo, r.puntos_xp FROM usuario_retos ur JOIN retos r ON r.id = ur.reto_id WHERE ur.usuario_id = ?');
    $stmt->execute([$usuario_id]);
    ...
    foreach ($stmt->fetchAll() as $ur) {
        ...
        if ($ur['completado'] == 1 && $nuevo_completado == 0) {
            $pdo->prepare('UPDATE usuarios SET puntos_xp = GREATEST(0, puntos_xp - ?) WHERE id = ?')->execute([$ur['puntos_xp'], $usuario_id]);
        }
        $pdo->prepare('UPDATE usuario_retos SET progreso = ?, completado = ?, fecha_completado = ? WHERE usuario_id = ? AND reto_id = ?')
            ->execute([$nuevo_progreso, $nuevo_completado, $nuevo_completado ? date('Y-m-d H:i:s') : null, $usuario_id, $ur['reto_id']]);
    }
 
    $stmt = $pdo->prepare('SELECT puntos_xp FROM usuarios WHERE id = ?');
    $stmt->execute([$usuario_id]);
    $xp_actual = $stmt->fetch()['puntos_xp'];
    $pdo->prepare('UPDATE usuarios SET nivel = ? WHERE id = ?')->execute([calcular_nivel($xp_actual), $usuario_id]);
 
    $ya_vista = false;
    $mensaje = '↩️ Película eliminada de tu historial.';
}
 
// Añadir/quitar pendientes
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_pendiente'])) {
    if ($en_pendientes) {
        $pdo->prepare('DELETE FROM lista_pendientes WHERE usuario_id = ? AND pelicula_id = ?')->execute([$usuario_id, $tmdb_id]);
        $en_pendientes = false;
        $mensaje = '🗑️ Eliminada de tu lista de pendientes.';
    } else {
        $pdo->prepare('INSERT IGNORE INTO lista_pendientes (usuario_id, pelicula_id) VALUES (?, ?)')->execute([$usuario_id, $tmdb_id]);
        $en_pendientes = true;
        $mensaje = '📌 Añadida a tu lista de pendientes.';
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_valoracion'])) {
    ...
    if ($puntuacion < 1 || $puntuacion > 5) {
        ...
        if ($mi_valoracion) {
            $pdo->prepare('UPDATE valoraciones SET puntuacion = ?, comentario = ?, fecha = NOW() WHERE usuario_id = ? AND pelicula_id = ?')
                ->execute([$puntuacion, $comentario, $usuario_id, $tmdb_id]);
            $mensaje_valoracion = '✅ Valoración actualizada correctamente.';
        } else {
            $pdo->prepare('INSERT INTO valoraciones (usuario_id, pelicula_id, puntuacion, comentario) VALUES (?, ?, ?, ?)')
                ->execute([$usuario_id, $tmdb_id, $puntuacion, $comentario]);
            $mensaje_valoracion = '✅ Valoración guardada correctamente.';
 
            $stmt = $pdo->prepare('SELECT COUNT(*) as total FROM valoraciones WHERE usuario_id = ?');
            $stmt->execute([$usuario_id]);
            $total_val = $stmt->fetch()['total'];
            $stmt = $pdo->prepare('SELECT r.id, r.objetivo FROM retos r LEFT JOIN usuario_retos ur ON ur.reto_id = r.id AND ur.usuario_id = ? WHERE r.tipo = "valoraciones" AND (ur.completado IS NULL OR ur.completado = 0)');
            $stmt->execute([$usuario_id]);
            foreach ($stmt->fetchAll() as $reto) {
...
$stmt = $pdo->prepare('SELECT v.*, u.nombre FROM valoraciones v JOIN usuarios u ON u.id = v.usuario_id WHERE v.pelicula_id = ? ORDER BY v.fecha DESC');
        ...
        <div class="detalle-info">
            ...
            <?php if ($trailer_key): ?>
                <div class="trailer-container">
                    ...
                    <iframe src="https://www.youtube.com/embed/<?= htmlspecialchars($trailer_key) ?>?rel=0"
            ...
            <div class="botones-pelicula">
                <form method="POST" style="display:inline">
                    ...
                    <?php if (!$ya_estrenada): ?>
                        <p class="no-estrenada">🗓️ Aún no estrenada — disponible el <?= date('d/m/Y', strtotime($fecha_estreno)) ?></p>
                    <?php elseif (!$ya_vista): ?>
                        <button type="submit" name="marcar_vista" class="btn-vista">🎬 Marcar como vista</button>
                    <?php else: ?>
                        <button type="submit" name="quitar_vista" value="1" class="btn-vista quitar">❌ Quitar de vistas</button>
                    <?php endif; ?>
                </form>
 
                <?php if ($ya_estrenada && !$ya_vista): ?>
                <form method="POST" style="display:inline">
                    <button type="submit" name="toggle_pendiente" class="btn-pendiente <?= $en_pendientes ? 'activo' : '' ?>">
                        <?= $en_pendientes ? '📌 En tu lista' : '+ Añadir a pendientes' ?>
                    </button>
                </form>
                <?php endif; ?>
            </div>
            ...
            <?php foreach ($valoraciones as $v): ?>
                <div class="resena-card <?= $v['usuario_id'] == $usuario_id ? 'mi-resena' : '' ?>">
                    <div class="resena-header">
                        ...
                        <span class="resena-estrellas"><?= str_repeat('★', $v['puntuacion']) ?><?= str_repeat('☆', 5 - $v['puntuacion']) ?></span>
...
<script>
setTimeout(() => { const t = document.getElementById('toast-reto'); if (t) t.style.display = 'none'; }, 4000);
</script>
